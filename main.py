...
class UI:
    ...
    def entry(self, parent, show=''):
        x = ttk.Entry(parent, show=show); x.pack(fill='x'); return x
    def button(self, parent, text, command):
        x = ttk.Button(parent, text=text, command=command); x.pack(side='left', padx=3); return x
    def clear(self):
...
class App(UI):
    ...
    def login_page(self):
        ...
        for text, name, show in [('Логин', 'login_entry', ''), ('Пароль', 'password_entry', '*')]:
            self.label(box, text).pack(anchor='w')
            entry = self.entry(box, show)
        ...
        for text, command in [('Войти', self.login), ('Сбросить', self.reset)]: self.button(buttons, text, command)
    ...
    def admin_page(self):
        ...
        for row, (text, name, show) in enumerate([('Логин', 'edit_login', ''), ('Пароль', 'edit_password', '*')]):
            self.label(form, text).grid(row=row, column=0)
            entry = ttk.Entry(form, show=show); entry.grid(row=row, column=1, sticky='ew'); setattr(self, name, entry)
        ...
        for text, command in [('Изменить', self.edit), ('Разблокировать', self.unlock), ('Удалить', self.delete), ('Выйти', self.login_page)]:
